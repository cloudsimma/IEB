void update_attendee(int portalid, int Attendeeid, map attend_details)
{
...
info creator_event;
Attende_id = creator_event.get("data").get(0).get("ID");
otherparams = Map();
response = zoho.creator.updateRecord("inspectorempirebuilder","ieb","All_Attendee_Details",Attende_id,datamap,otherparams,"creator");
info response;
}
